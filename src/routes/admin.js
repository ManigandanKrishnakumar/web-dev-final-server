...
adminRouter.put(
    ROUTES.ADMIN.ADMIN_UPDATES_USER,
    authenticateToken,
    async (req, res) => {
        const { username, userRole, meta_data } = req.body;

        try {
            const result = await adminUpdatesUser(
                username,
                userRole,
                req.decodeduserRole,
                meta_data
...
adminRouter.delete(
    ROUTES.ADMIN.ADMIN_DELETES_USER,
    authenticateToken,
    async (req, res) => {
        const { username } = req.body;
        try {
            const result = await adminDeletesUser(
                username,
                req.decodeduserRole
            );
            res.status(200).json(result);
